...
기본 앱은 별도 NASM 오브젝트 11개를 GNU ld로 직접 연결한다. C를 어셈블리로 변환한 코드, 정적 libc, CRT, 수학 라이브러리를 포함하지 않았다. Linux 커널·파일시스템·터미널 의존성은 허용한다. 데이터는 기존 고정 BSS이며 범용 메모리 할당기는 추가하지 않았다.
...
기존 수학과 SSE2 커널의 소스는 바꾸지 않았다. 코어에서는 출력 함수 이름을 `rt_console_format`으로 명확히 하고, 평가 직전 FP 상태 초기화·스크립트 close 실패 처리를 추가했다. 새 포매터가 지원하는 것은 기존 화면이 사용하는 정해진 형식 부분집합이며 전체 printf 호환 기능은 아니다.
